...
// ============================================
// LOCALE REDIRECT (antes en proxy.ts)
// ============================================
...
function handleLocale(request: NextRequest): NextResponse | null {
  ...
  if (pathnameHasLocale) return null; // sin redirección
...
// ============================================
// MATCHER — unión de ambos matchers
// ============================================
export const config = {
  matcher: [
    // Rate limiting — rutas API
    "/api/admin/auth",
    "/api/drive/:path*",
    "/api/image/:path*",
    "/api/properties",
    "/api/leads/:path*",
    // Locale redirect — todo excepto assets estáticos y API
...
export async function middleware(req: NextRequest) {
  ...
  // 1. Si es ruta API — aplicar rate limiting
  ...
  // 2. Si es ruta de página — locale redirect
